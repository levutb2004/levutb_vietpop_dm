# src/vietpop/core/dasymetric.py
import time
import numpy as np
import pandas as pd
...
class DasymetricMapper:
    ...
    def _calculate_normalization(self,
        ...
        if self.settings.district_dm:
            mastergrid = self.settings.district_mastergrid
    ...
    def _create_normalized_raster(self,
        ...
        if constrained:
            ...
            if self.settings.district_dm:
                mastergrid = self.settings.district_mastergrid
            output_path = Path(self.settings.output_raster['normalization'])
    ...
    def _create_dasymetric_raster(self,
        ...
        if (constrained):
            ...
            output_path = Path(self.settings.output_raster['dasymetric'])
        if suffix:
            output_path = Path(str(output_path).replace('.tif', f'_{suffix}.tif'))
            #output_path = output_path.replace('.tif', f'_{suffix}.tif')
    ...
    def map(self,
        ...
        # for each admin id, norm = pop / sum of prediction values in that admin unit
        ...
        # Create normalized raster
        # write to .tif the norm value for each pixel. All pixels of the same admin id have the same norm value
        ...
        # pop at pixel = norm at pixel * prediction at pixel
    ...
    def map_district(self,
        ...
        # Load and validate inputs
        self._validate_inputs(prediction_path, self.settings.district_mastergrid, self.settings.constrain)

        # Load census data
        census, id_column, pop_column = self._load_census(
            self.settings.district_census['path'],
            **self.settings.district_census
        )
